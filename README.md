...
It is currently 1930 bytes in size *before* minification and zipping!
...
Limitations
===========
The HTML generated by Baremark is not necessarily identical to that of other
Markdown modules. For one, Baremark favors older HTML tags, such as `<i>`,
`<b>` and `<s>` (over the newer `<em>`, `<strong>` and `<del>` etc) because
they’re shorter. (*Semantic markup, pha!*—Who are we kidding anyway?!) Also
Baremark does not spend any effort on collapsing or cleaning up whitespace (the
browser collapses them all anyway), so for the most part, the whitespace found
in the outputted HTML is the same as the one found in the Markdown (with the
obvious exceptions being the processing of newlines and linebreaks mandated by
the Markdown specs). For source brevity Baremark uses HTML character code
entities (such as `&#38;`), rather than more commonly seen named entities
(`&amp;`).

CommonMark / Baremark differences (some of these may change in the future):

* Baremark does not have a concept of “block” or “inline” tags (which
  admittedly might be a failure on its part). This helps keeping its source
  code short, but also means that one needs to write each Baremark rule *very*
  carefully for them not to step on each others toes.

* Indentation and tabs are ignored (what goes in comes out, except that tabs
  and spaces are trimmed at end-of-line). Tabs are not recognized in Markdown
  tags instead of spaces.

* ` ```↲CODEBLOCK↲``` ` (fenced [code blocks]) are supported, but indented code
  blocks are not.

* [Lists] cannot be nested.

* `<URL>` autolinks are not supported.

* Determining what is a [paragraph] and what is a [HTML block] from is somewhat
  simplistic.

* Whitespace is *not* allowed between `](` or `][` in [links and images]. This
  allowed in the [CommonMark] specification, but can lead to weird errors.
  (Baremark allows you to space inside the brackets though, so your line
  wrapping shouldn’t be too affected.)

* `&ENT;` (HTML entities) in the Markdown input are left exactly as-is. `\X`
  (backslash escapes) and `` `CODE` `` and ` ```↲CODEBLOCK↲``` ` escape
  characters into `&#NUM;` (numeric HTML entities) to make sure that they are
  ignored by later rules.—`\X` does this to *any* character, while ``CODE`` and
  ````↲CODEBLOCK↲```` does this only to ``'\<>`*~_=:"![]()\n\t-``.

* `# HEADING` (ATX headings) and `---` `___` `***` (dinkuses) must be separated
  from surrounding text by blank lines, while Setext headings `HEADING↲=======`
  `HEADING↲-------` must be *preceded* by a blank line (but does not have to be
  followed by one).
