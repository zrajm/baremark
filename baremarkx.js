(w=>{
	let l,t=x=>`((?:${x}|\n(?!\n))*?)`,b=(x,p=r[1])=>p.reduce((a,r)=>a.replace(...r),x),m={N:'\n',X:t('.'),Y:t('[^()[\\]\n]')},s=x=>x.trim().replace(/\s+/g,' '),q=x=>`&#${x.charCodeAt()};`,e=(x,r='&#|')=>x.replace(RegExp(`[${r}'\\\\<>\`*~_=:"![\\]()\n\t-]`,'g'),q),h=(z,x,u,a='')=>(u=e(s(u),''),a=e(a,''),z?`<img src="${u}" title="${a}" alt="${e(x,'')}">`:`<a href="${u}" title="${a}">${x}</a>`),y=z=>(w,x,r,u)=>u?h(z,x,u):(r=l[n(r||x)],r?h(z,x,...r):w),n=x=>s(x).toLowerCase(),r=[[
		...
		[/NN([-_*])( *\1){2,}(?=NN)/,'\n\n<hr>'],
		[/N(?:[-+*]|(\d+)[.)]) +(.+(N .+)*)/,(_,i,x)=>`\n\n<${i=i?'ol':'ul'}><li>${b(x)}</li></${i}>\n`],
		[/<\/(ol|ul)>NNN<\1>/,''],
		[/NN(#{1,6}) +(\S.*?)( +#+)?(?=NN)/,(_,i,x)=>`\n\n<h${i=i.length}>${b(x)}</h${i}>`],
		[/N(.+?(?:N.+?)*?)N(?:(=+)|-+)(?=N)/,(_,x,i)=>`\n<h${i=i?1:2}>${b(x)}</h${i}>\n`],
		[/NN(.+(N.+)*)(?=NN)/,(w,x)=>/<\/?(address|article|aside|blockquote|details|div|[dou]l|fieldset|fig(caption|ure)|footer|form|h[1-6r]|header|hgroup|main|menu|nav|p|pre|(no)?script|search|section|style|table)\b/i.test(x)?b(w):`\n\n<p>${b(x)}</p>`]
	],[
		[/!\[Y\](?:\[Y\]|\(Y\))?/,y(1)],
		[/\[Y\](?:\[Y\]|\(Y\))?/,y()],
		[/___X___/,'<u>$1</u>'],
		...
		[/~~X~~/,'<s>$1</s>'],
		[/:"X":/,'<q>$1</q>'],
	]].map(r=>r.map(([r,s])=>[RegExp(r.source.replace(/[NXY]/g,x=>m[x]),'g'),s]))
